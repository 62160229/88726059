function formValidation(){
    ...
    var usex = document.registform.sez;

    if(validateUserID(uid, 5, 12)){
        if(validateUserID(passid, 7, 12)){
            if(allLetter(uname)){
...
function validateUserID(uid, min, max){
    var error = "";
    var illegalChars = /\W/; //allow ketters, numbers, and underscores
    ...
    if(uid.value ==""){
        ...
        uid.style.background = 'Yellow';
        error = "User ID มีตัวอักษรที่ไม่ได้รับอนุณาต\n";
        alert(error);
        uid.focus();
        return false;
    } else{
...
function validatePassword(passid, min, max){
    ...
    if(passid.value == ""){
        ...
        error = "Password มีตัวอักษรที่ไม่ได้รับอนุณาต\n";
        passid.style.background = 'Yellow';
        alert(error);
        passid.focus();
        return false;
    } else if((passid.value.search(/[a-zA-Z]+/) == -1) || (passid.style.search(/[0-9]+/) == -1)){
